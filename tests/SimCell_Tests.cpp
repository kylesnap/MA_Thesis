...
TEST_CASE("Test Design Mat Construction") {
    ...
    std::tuple<float, float, float> t1 = {0, 0, 0};
    std::tuple<float, float, float> t2 = {1, 0, 1};

    std::vector<float> v;
    SimCell tst1 = SimCell(100, 0.8, b1, b1, p1, t1, r2);
    tst1.getSimCell(v);
    // N = 100, RSQ = 0.8, (A, B) = (25, 25), PROP = (50, 50, 0) THETA = 0, 0, 0
    CHECK_THAT(v, Catch::Matchers::Equals(std::vector<float>{100, 50, 50, 0, 25, 25, 25, 25, 0, 0, 0, 0.8}));

    v.clear();
    SimCell tst2 = SimCell(200, 0.5, b2, b2, p2, t2, r2);
    tst2.getSimCell(v);
    // N = 200, RSQ = 0.5, (A, B) = (10.5, 29.5), PROP = (50, 80, 70), THETA = 1, 0, 1
    CHECK_THAT(v, Catch::Matchers::Equals(std::vector<float>{200, 50, 80, 70, 10.5, 29.5, 10.5, 29.5, 1, 0, 1, 0.5}));

    v.clear();
    SimCell tst3 = SimCell(420, 0.69, b1, b2, p1, t2, r2);
    tst3.getSimCell(v);
    // N = 420, RSQ = 0.69, BQ/BP are different, PROP = (210, 210, 0), THETA = 1, 0, 1
    CHECK_THAT(v, Catch::Matchers::Equals(std::vector<float>{420, 210, 210, 0, 25, 25, 10.5, 29.5, 1, 0, 1, 0.69}));
}