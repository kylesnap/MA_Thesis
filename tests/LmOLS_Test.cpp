...
const gsl_rng_type *lmT = gsl_rng_default;
gsl_rng *lmr = gsl_rng_alloc(lmT);
...
TEST_CASE("Test norm LM with Data") {

    gsl_matrix * X = gsl_matrix_alloc(10000, 3);
    gsl_vector * Y = gsl_vector_alloc(10000);
    ...
    gsl_vector_view tempX = gsl_matrix_column(X, 0);
    gsl_vector_set_all(&tempX.vector, 1);
    for (int i = 0; i < X->size1; i++) {
        gsl_matrix_set(X, i, 1, gsl_ran_ugaussian(lmr));
        gsl_matrix_set(X, i, 2, gsl_ran_ugaussian(lmr));
        gsl_vector_set(Y, i, gsl_ran_gaussian(lmr, 0.5));
    }

    gsl_blas_dgemv(CblasNoTrans, 1.0, X, B, 1.0, Y); // No error terms.
    ...
    mod.getBetaHat(betas);
    REQUIRE_THAT(betas,
                 Catch::Matchers::Approx(std::vector<float>{1, 0.5, -1}).margin(0.01));

    std::vector<float> betaN;
    mod.getBetaHat(betaN, true);
    REQUIRE_THAT(betaN,
                 Catch::Matchers::Approx(std::vector<float>{0, 0.41, -0.82}).margin(0.01));
}